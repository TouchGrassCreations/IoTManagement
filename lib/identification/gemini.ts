...
const prompt = `Identify one or more distinct IoT, electronics, mechanical, or robotics inventory items in this image. Deliberately scan the entire frame, including every edge and corner, before responding. Include partially visible or cropped items when distinctive visible features support their general identity; reduce confidence and set model to null when the exact model is not supported. An item touching or extending beyond an image boundary must not be omitted solely because it is cropped.
...
export async function recognizeComponents(input: { bytes: Uint8Array; mimeType: string; fetchImpl?: typeof fetch; apiKey?: string; model?: string }): Promise<Detection[]> {
  ...
  const response = await (input.fetchImpl ?? fetch)(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${encodeURIComponent(key)}`, {
    ...
    body: JSON.stringify({ contents: [{ parts: [{ text: prompt }, { inlineData: { mimeType: input.mimeType, data: base64 } }] }], generationConfig: { responseMimeType: "application/json" } }),
  });
  if (!response.ok) {
